...
class Checkout extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      restaurantDetails: props.location.restaurantDetails
        ...
        : { name: null }, //Setting state with details passed from the previous page
      coupon: null,
      couponName: "",
  ...
  //This method is called every time the page is rendered to check if the customer is logged in and cartItems is not empty if not then redirected to the home page.
  redirectToHome = () => {
    if (!this.state.isLoggedIn) {
      return <Redirect to="/" />;
    }
          ...
          <div className="stepper-container">
            ...
            >
              {this.state.steps.map((
                label,
                index //Iteration over each step to create a stepper
              ) => (
                <Step key={label}>
